...
Clients are expected to build RequestMessages and wrap them in SeradMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a SeradMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a SeradMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network serad is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which sera address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether serad thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when serad is not synced. That is because when serad isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known serad addresses in the
...
currently connected to this serad.
...
| isOutbound | [bool](#bool) |  | Whether this serad initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this serad |
...
AddPeerRequestMessage adds a peer to serad&#39;s outgoing connection list.
This will, in most cases, result in serad connecting to said peer.
...
parent chain from some startHash to this serad&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this serad.
...
ShutDownRequestMessage shuts down this serad.
...
This call is only available when this serad was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given serad addresses
